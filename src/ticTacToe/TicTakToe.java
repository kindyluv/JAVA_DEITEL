...
import static ticTacToe.Players.PLAYER1;
import static ticTacToe.Players.PLAYER2;



public class TicTakToe {
    private CellElement cellElement = CellElement.EMPTY;
    ...
    public void currentPlayerMarks(Players player, int position) {
        if (position > 0 && position < 10) {
            if (player == PLAYER1) setPosition(position, CellElement.X);
            else if (player == PLAYER2) setPosition(position, CellElement.O);
        }
    ...
    private void validateAndAcceptPlayerPosition(int rowPosition, int columnPosition, CellElement board) {
        if (!playArray[rowPosition][columnPosition].equals(CellElement.EMPTY.name())) throw new IllegalArgumentException("Position is already filled");
    ...
    public StringBuilder printOnBoard() {
        ...
        for (String[] rows : playArray) {
            ...
            for (String column : rows) {
                if (column.equals(String.valueOf(CellElement.EMPTY))) displayPlayboard.append("_");
